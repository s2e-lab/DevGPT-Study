...
def is_java_file(filename):
    """ Check if the given filename is a Java file. """
    return filename.endswith('.java')


def copy_json_if_java_files(json_file, dest_dir):
    """ Copy the JSON file to the destination directory if it contains Java files. """
    with open(json_file, 'r') as file:
        ...
        if any(is_java_file(f) for f in updated_files):
            print(f"Java file found in {json_file}. Copying to destination.")
...
for json_file in list_json_files(source_dir):
    copy_json_if_java_files(json_file, dest_dir)
